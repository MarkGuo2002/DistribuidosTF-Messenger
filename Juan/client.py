...
class client :
    ...
    def  register(user, window):
        client._socket.sendall(user.encode())
        answer_bytes = client._socket.recv(1024)
    ...
    def  send(user, message, window):
        ...
        answer = 0
        if answer == "0": 
            window['_SERVER_'].print("s> SEND OK - MESSAGE {}".format(id)) # Revisar
        elif answer == "1": 
            window['_SERVER_'].print("s> SEND FAIL / USER DOES NOT EXIST")
        elif answer == "2": 
            window['_SERVER_'].print("s> SEND FAIL")

        # Hay algo más, lo tengo que revisar
    ...
    def  connectedUsers(window):
        ...
        answer = 0
        if answer == "0": 
            window['_SERVER_'].print("s> CONNECTED USERS (N users connected) OK - <user1>, <user2>, ... <userN>") # Revisar
        elif answer == "1": 
